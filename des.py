...
PC_2 = [14, 17, 11, 24, 1, 5, 3, 28, 15, 
6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 
7, 27, 20, 13, 2, 41, 52, 31, 37, 47, 55, 
...
def encrypt_block(blk, subkeys,y):
    assert len(blk)==8
    ...
    for i in range(ROUNDS):
        left = bits[:32]
        right = bits[32:]
        left = xor_bits(left, F(right, subkeys[(i+y)%16]))
        bits = right + left
        #print 'ct_i :'+ bits_to_str(bits)
        e = left+right
        e = [e[x-1] for x in IP_1]
        print '[+]round '+str(i+1)+':'
        print 'ct_i :'+ bits_to_str(e)
    bits = left + right
    bits = [bits[x-1] for x in IP_1]
...
'''
def gen_subkey():
    key_map='e0be661032d5f0b676f82095e4d67623628fe6d376363183aed373a60167af537b46abc2af53d97485591f5bd94b944a3f49d94897ea1f699d1cdc291f2d9d4a5c705f2cad89e938dbacaca15e10d8aeaed90236f0be2e954a8cf0bea6112e84'.decode('hex')
    subkeys = []
    for i in range(16):
        a = str_to_bits(key_map[i:i+12])
        #print a
        #b = bin(int(key_map[i:i+6].encode('hex'),16))[2:].rjust(48,'0')
        #b = map(int,list(b))
        #print b
        #print a == b
        subkeys.append(a)
...
def decrypt_block(blk, subkeys,y):
    assert len(blk)==8
    bits = str_to_bits(blk)
    #bits = [bits[x-1] for x in IP]
    left = bits[:32]
    ...
    for i in range(ROUNDS):
        left = bits[:32]
        right = bits[32:]
        left = xor_bits(left, F(right, subkeys[(ROUNDS+y-1-i)%16]))
        bits = right + left
        print bits
    bits = left + right
    bits = [bits[x-1] for x in IP_1]
...
def decrypt(ct):
    subkeys = gen_subkey()
    for y in range(16):
        pt = encrypt_block(ct, subkeys,y)
    return pt

ct = "\x92\xd9\x15\x25\x01\x19\xe1\x2b"
